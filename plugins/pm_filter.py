    ...
    if ENABLE_SHORTLINK and settings['button']:
        ...
        btn = [
            [
                ...
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)} ᠰ {file.file_name}", url=f"https://telegram.dog/{temp.U_NAME}?start=aNsH_{file.file_id}"
                ),
            ]
            for file in files
        ]
...
async def season_check(bot, query):
...
async def select_season(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Lᴀɴɢᴜᴀɢᴇ ↓", callback_data=f"season#{userid}#unknown")
    ],[
        InlineKeyboardButton("Season 01", callback_data=f"season#{userid}#s01"),
        InlineKeyboardButton("Season 02", callback_data=f"season#{userid}#s02")
    ],[
        InlineKeyboardButton("Season 03", callback_data=f"season#{userid}#s03"), 
        InlineKeyboardButton("Season 04", callback_data=f"season#{userid}#s04")
    ],[
        InlineKeyboardButton("Season 05", callback_data=f"season#{userid}#s05"),
        InlineKeyboardButton("Season 06", callback_data=f"season#{userid}#s06")
    ],[
        InlineKeyboardButton("Season 07", callback_data=f"season#{userid}#s07"), 
        InlineKeyboardButton("Season 08", callback_data=f"season#{userid}#s08")
    ],[
        InlineKeyboardButton("Season 09", callback_data=f"season#{userid}#s09"),
        InlineKeyboardButton("Season 10", callback_data=f"season#{userid}#s10")
    ],[
        InlineKeyboardButton("Season 11", callback_data=f"season#{userid}#s11"), 
        InlineKeyboardButton("Season 12", callback_data=f"season#{userid}#s12")
    ],[
        InlineKeyboardButton("Gᴏ Bᴀᴄᴋ", callback_data=f"season#{userid}#home")
    ]]
    try:
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        ...
        InlineKeyboardButton("! Lᴀɴɢᴜᴀɢᴇs !", callback_data=f"select_season#{message.from_user.id}")
    ])
    btn.insert(0, [
        InlineKeyboardButton("⚡ Cʜᴇᴄᴋ Bᴏᴛ PM ⚡", url=f"https://t.me/{temp.U_NAME}")
    ])
